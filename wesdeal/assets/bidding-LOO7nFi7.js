import{h as t,s as L}from"./nav-aRKDhcJG.js";import{r as M}from"./calendarView-DIOHmfzy.js";function S(o){const a=[],e=/(\*\*[^*]+\*\*|`[^`]+`|\[[^\]]+\]\([^)]+\))/g;let n=0;for(const h of o.matchAll(e)){h.index>n&&a.push({t:"text",s:o.slice(n,h.index)});const r=h[0];if(r.startsWith("**"))a.push({t:"bold",s:r.slice(2,-2)});else if(r.startsWith("`"))a.push({t:"code",s:r.slice(1,-1)});else{const p=/^\[([^\]]+)\]\(([^)]+)\)$/.exec(r);a.push({t:"link",s:p[1],href:p[2]})}n=h.index+r.length}return n<o.length&&a.push({t:"text",s:o.slice(n)}),a}function B(o){return o.trim().replace(/^\|/,"").replace(/\|$/,"").split("|").map(e=>e.trim())}function R(o){return/^\|?[\s:|-]+\|?$/.test(o.trim())&&o.includes("-")}function E(o){const a=[],e=o.split(`
