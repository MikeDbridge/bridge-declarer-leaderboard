import{h as t,s as L}from"./nav-aRKDhcJG.js";import{r as M}from"./calendarView-Cqg0of4Y.js";function S(o){const a=[],e=/(\*\*[^*]+\*\*|`[^`]+`|\[[^\]]+\]\([^)]+\))/g;let n=0;for(const h of o.matchAll(e)){h.index>n&&a.push({t:"text",s:o.slice(n,h.index)});const r=h[0];if(r.startsWith("**"))a.push({t:"bold",s:r.slice(2,-2)});else if(r.startsWith("`"))a.push({t:"code",s:r.slice(1,-1)});else{const p=/^\[([^\]]+)\]\(([^)]+)\)$/.exec(r);a.push({t:"link",s:p[1],href:p[2]})}n=h.index+r.length}return n<o.length&&a.push({t:"text",s:o.slice(n)}),a}function B(o){return o.trim().replace(/^\|/,"").replace(/\|$/,"").split("|").map(e=>e.trim())}function R(o){return/^\|?[\s:|-]+\|?$/.test(o.trim())&&o.includes("-")}function E(o){const a=[],e=o.split(`
...
`)});continue}if(r.startsWith("|")){const s=B(h).map(S);n++,n<e.length&&R(e[n])&&n++;const i=[];for(;n<e.length&&e[n].trim().startsWith("|");)i.push(B(e[n]).map(S)),n++;a.push({t:"table",header:s,rows:i});continue}if(h.startsWith("- ")){const s=[h.slice(2)];for(n++;n<e.length;){const i=e[n];if(i.startsWith("- "))s.push(i.slice(2)),n++;else if(/^\s{2,}\S/.test(i))s[s.length-1]+=" "+i.trim(),n++;else break}a.push({t:"ul",items:s.map(S)});continue}const p=[r];for(n++;n<e.length;){const s=e[n],i=s.trim();if(i===""||s.startsWith("#")||s.startsWith("- ")||i.startsWith("|")||i.startsWith("```"))break;p.push(i),n++}a.push({t:"p",inline:S(p.join(" "))})}return a}function j(o){return o.toLowerCase().replace(/[^a-z0-9]+/g,"-").replace(/^-+|-+$/g,"")}const W=/^(<(\d+)|(\d+)\+|(\d+)):(\d+)%$/;function O(o){const a=o.trim().split(/\s+/);if(a.length<2)return null;const e=[],n=[];for(const h of a){const r=W.exec(h);if(!r){if(e.length>0)return null;n.push(h);continue}const p=Number(r[5]);r[2]!==void 0?e.push({label:`<${r[2]}`,anchor:Number(r[2])-1,pct:p}):r[3]!==void 0?e.push({label:`${r[3]}+`,anchor:Number(r[3]),pct:p}):e.push({label:r[4],anchor:Number(r[4]),pct:p})}return e.length<2?null:{prefix:n.join(" "),tokens:e}}const $=`# What championship players actually hold for every bid
...
`;function m(o){const a=[];let e="";for(const n of o)n==="♥"||n==="♦"?(e&&(a.push(e),e=""),a.push(t("span",{class:"suit-red"},[n]))):e+=n;return e&&a.push(e),a}function g(o){const a=[];for(const e of o)e.t==="text"?a.push(...m(e.s)):e.t==="bold"?a.push(t("strong",{},m(e.s))):e.t==="link"?a.push(t("a",{href:e.href,class:"rep-link"},[e.s])):a.push(t("code",{class:"rep-code-inline",title:"Click to copy"},[e.s]));return a}const Q=/^[\d\s.,%()\/±×–—+*-]*\d[\d\s.,%()\/±×–—+*-]*$/;function I(o){if(o.some(e=>e.t==="code"))return!1;const a=o.map(e=>e.s).join("");return Q.test(a.replace(/\*\*/g,"").trim())}function K(o,a,e){const n=[],h=new Map(o.tokens.map(s=>[s.anchor,s])),r=Math.max(...o.tokens.map(s=>s.pct));for(let s=a;s<=e;s++){const i=h.get(s);if(!i){n.push(t("span",{class:"dist-slot dist-empty"},[]));continue}const u=.08+.5*(i.pct/100);n.push(t("span",{class:"dist-slot"+(i.pct===r?" dist-peak":""),style:`background: rgba(29,111,66,${u.toFixed(3)})`,title:`${i.label}: ${i.pct}%`},[t("span",{class:"dist-label"},[i.label]),t("span",{class:"dist-pct"},[String(i.pct)])]))}const p=[];return o.prefix!==""&&p.push(t("span",{class:"dist-prefix"},m(o.prefix))),p.push(t("span",{class:"dist-grid"},n)),t("span",{class:"dist"},p)}function G(o,a){const e=t("thead",{},[t("tr",{},o.map(i=>t("th",{},g(i))))]),n=o.length,h=a.map(i=>i.map(u=>u.some(f=>f.t!=="text")?null:O(u.map(f=>f.s).join("")))),r=new Array(n).fill(1/0),p=new Array(n).fill(-1/0);for(const i of h)for(let u=0;u<i.length;u++){const f=i[u];if(f)for(const b of f.tokens)r[u]=Math.min(r[u],b.anchor),p[u]=Math.max(p[u],b.anchor)}const s=t("tbody",{},a.map((i,u)=>t("tr",{},i.map((f,b)=>{const H=h[u][b];return H&&p[b]-r[b]<=11?t("td",{class:"dist-td"},[K(H,r[b],p[b])]):t("td",{class:I(f)?"num":!1},g(f))}))));return t("div",{class:"rep-table-wrap"},[t("table",{class:"rep-table"},[e,s])])}function U(o){switch(o.t){case"p":return t("p",{class:"rep-p"},g(o.inline));case"ul":return t("ul",{class:"rep-ul"},o.items.map(a=>t("li",{},g(a))));case"table":return G(o.header,o.rows);case"code":return t("pre",{class:"rep-code"},[o.text]);default:return t("p",{},[])}}function J(o){const a=o.find(d=>d.t==="h1"),e=t("div",{},[]),n=t("div",{class:"bids-main"},[]),h=[],r=new Set,p=d=>{let l=j(d)||"section",c=2;for(;r.has(l);)l=`${j(d)}-${c++}`;return r.add(l),l};let s=null,i=null;const u=d=>{const l=U(d);i?i.el.append(l):s?(s.el.append(l),s.hasOwnContent=!0):n.append(l),i&&(i.searchText+=" "+V(d))};for(const d of o)if(d.t!=="h1"){if(d.t==="h2"){const l=p(d.text),c=t("a",{href:`#${l}`},m(d.text));e.append(c);const v=t("section",{class:"rep-section",id:l},[t("h2",{},m(d.text))]);n.append(v),s={el:v,tocLink:c,subs:[],hasOwnContent:!1},h.push(s),i=null;continue}if(d.t==="h3"){const l=p(d.text),c=t("details",{class:"rep-sub",id:l,open:!0},[t("summary",{},m(d.text))]),v=t("a",{href:`#${l}`,class:"toc-h3"},m(d.text));e.append(v),((s==null?void 0:s.el)??n).append(c),i={el:c,tocLink:v,searchText:d.text.toLowerCase()},s==null||s.subs.push(i);continue}u(d)}const f=t("input",{class:"bids-filter",type:"search",placeholder:'Filter contexts… e.g. "1S (2H)", "balance", "X"'}),b=t("span",{class:"bids-count"},[]),H=t("button",{type:"button"},["Expand all"]),D=t("button",{type:"button"},["Collapse all"]),X=d=>{for(const l of h)for(const c of l.subs)c.el.open=d};H.addEventListener("click",()=>X(!0)),D.addEventListener("click",()=>X(!1));const T=()=>{const d=f.value.trim().toLowerCase();let l=0,c=0;for(const v of h){let q=!1;for(const y of v.subs){c++;const P=d===""||y.searchText.includes(d);y.el.classList.toggle("hidden",!P),y.tocLink.classList.toggle("hidden",!P),P&&(l++,q=!0,d!==""&&(y.el.open=!0))}const k=d===""||q||v.hasOwnContent;v.el.classList.toggle("hidden",!k),v.tocLink.classList.toggle("hidden",!k)}b.textContent=d===""?`${c} contexts`:`${l} of ${c} contexts`};f.addEventListener("input",T);const C=t("div",{class:"bids-toast"},[]);let w=0;n.addEventListener("click",d=>{var v;const l=d.target.closest("code.rep-code-inline");if(!l)return;const c=l.textContent??"";(v=navigator.clipboard)==null||v.writeText(c).then(()=>{C.textContent=`Copied: ${c}`,C.classList.add("show"),window.clearTimeout(w),w=window.setTimeout(()=>C.classList.remove("show"),1600)})});const N=document.getElementById("app");N.classList.add("bids-app");const F=t("aside",{class:"bids-toc"},[t("p",{class:"bids-toc-title"},["Contents"]),e]),x=M(),A=t("details",{class:"bids-datamap"},[t("summary",{},[t("span",{class:"bids-datamap-title"},["Tournament data"]),t("span",{class:"bids-datamap-sub"},[x.summary])]),t("div",{class:"bids-datamap-body"},x.nodes)]);N.append(L("bidding"),t("header",{class:"app-header"},[t("h1",{},["WesData"]),t("p",{class:"tagline"},[a&&a.t==="h1"?a.text:"Bidding ranges from championship data"])]),A,t("div",{class:"bids-controls"},[f,H,D,b]),t("div",{class:"bids-layout"},[F,n]),C),T()}function V(o){switch(o.t){case"p":return o.inline.map(a=>a.s).join("").toLowerCase();case"ul":return o.items.map(a=>a.map(e=>e.s).join("")).join(" ").toLowerCase();case"table":return o.rows.map(a=>a.map(e=>e.map(n=>n.s).join("")).join(" ")).join(" ").toLowerCase();case"code":return o.text.toLowerCase();default:return o.text.toLowerCase()}}J(E($));
